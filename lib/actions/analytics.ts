...
export async function exportAnalyticsReport(
  ...
  const scoreStatus =
    analytics.complianceScore >= 80 ? "Excellent"        :
    analytics.complianceScore >= 60 ? "Good"             :
    analytics.complianceScore >= 40 ? "Needs Attention"  :
    "Non-Compliant";

  const dsarCompletionRate =
    analytics.dsarMetrics.total > 0
      ? Math.round((analytics.dsarMetrics.completed / analytics.dsarMetrics.total) * 100)
      : 0;

  const lines: string[] = [
    `# GDPR Compliance Analytics Report`,
    ``,
    `> **ComplianceKit**  ·  Generated: ${generated}  ·  Period: ${rangeLabel}`,
    ``,
    `---`,
    ``,
    `## Executive Summary`,
    ...
    `| Metric | Value |`,
    `|--------|-------|`,
    `| Overall Compliance Score | **${analytics.complianceScore}% — ${scoreStatus}** |`,
    `| Total Visitor Consents | ${analytics.consentMetrics.total} |`,
    `| Consent Acceptance Rate | ${analytics.consentMetrics.acceptanceRate}% |`,
    `| Compliance Scans Run | ${analytics.scanMetrics.totalScans} |`,
    `| Average Scan Score | ${analytics.scanMetrics.averageScore}% |`,
    `| DSAR Requests Received | ${analytics.dsarMetrics.total} |`,
    `| DSAR Completion Rate | ${dsarCompletionRate}% |`,
    ...
    `|----------|-------|-------|`,
    `| Accepted All | ${analytics.consentMetrics.acceptedAll} | ${analytics.consentMetrics.total > 0 ? Math.round((analytics.consentMetrics.acceptedAll / analytics.consentMetrics.total) * 100) : 0}% |`,
    `| Rejected All | ${analytics.consentMetrics.rejectedAll} | ${analytics.consentMetrics.total > 0 ? Math.round((analytics.consentMetrics.rejectedAll / analytics.consentMetrics.total) * 100) : 0}% |`,
    `| Partial Consent | ${analytics.consentMetrics.partial} | ${analytics.consentMetrics.total > 0 ? Math.round((analytics.consentMetrics.partial / analytics.consentMetrics.total) * 100) : 0}% |`,
    `| **Total** | **${analytics.consentMetrics.total}** | 100% |`,
    ``,
    `### Consent by Category`,
    ``,
    `| Category | Consents Granted |`,
    `|----------|-----------------|`,
    ...
    `| Average Compliance Score | ${analytics.scanMetrics.averageScore}% |`,
    `| Total Cookies Detected | ${analytics.scanMetrics.cookiesFound} |`,
    `| Tracking Scripts Detected | ${analytics.scanMetrics.scriptsFound} |`,
    `| Total Compliance Issues | ${analytics.scanMetrics.findingsCount} |`,
    ``,
    ...
    `| ℹ️ Info | ${analytics.scanMetrics.findingsBySeverity.info} |`,
    ``,
    `---`,
    ``,
    ...
    `| ⚠️ Overdue | ${analytics.dsarMetrics.overdue} |`,
    `| **Total** | **${analytics.dsarMetrics.total}** |`,
    ``,
    `- **Average response time:** ${analytics.dsarMetrics.averageResponseTime} days`,
    `- **Completion rate:** ${dsarCompletionRate}%`,
    ``,
    `### Requests by Type`,
    ``,
    `| Type | Count |`,
    `|------|-------|`,
    `| Access (Art. 15) | ${analytics.dsarMetrics.byType.access} |`,
    `| Erasure (Art. 17) | ${analytics.dsarMetrics.byType.erasure} |`,
    `| Rectification (Art. 16) | ${analytics.dsarMetrics.byType.rectification} |`,
    `| Portability (Art. 20) | ${analytics.dsarMetrics.byType.portability} |`,
    `| Restriction (Art. 18) | ${analytics.dsarMetrics.byType.restriction} |`,
    `| Objection (Art. 21) | ${analytics.dsarMetrics.byType.objection} |`,
    ``,
    `---`,
    ``,
    `## Notes`,
    ``,
    `- This report covers the period: **${rangeLabel}**`,
    `- All metrics are scoped to your ComplianceKit account.`,
    `- DSAR response deadlines are governed by GDPR Art. 12 (30-day statutory limit).`,
    `- This document is generated automatically and does not constitute legal advice.`,
    ``,
    `---`,
    ``,
    `*ComplianceKit  ·  https://compliancekit.app  ·  ${generated}*`,
  ];
